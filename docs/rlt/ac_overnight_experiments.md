...
|--------|-----|-------|---------|----------|----------|-------|--------|------|
| s2_b0.3_100k | 0.3 | 100K | 13.45 | 18.30 | 10.49 | 7.81 | 0.079 | 发散（比50K轻但仍发散） |
...
### 关键发现 5: 100K 步训练 β≤1.0 灾难性发散

β=1.0 在 50K 步稳定（ref_mse=2.5），但 100K 步直接爆炸（ref_mse=191, Q=371）。
β=0.3 在 100K 步也发散（ref_mse=13.45, 但比 50K 的 84.79 轻，说明前半程还在收敛）。

结论：低 β offline RL 在固定 buffer 上长训必然发散（Q 值过估计正反馈循环）。β≥3.0 是安全区间。
