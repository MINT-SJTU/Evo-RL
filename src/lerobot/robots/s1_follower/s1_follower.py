...
class S1Follower(Robot):
    ...
    def __init__(self, config: S1FollowerConfig):
        ...
        self.arm = None  # 延迟到 connect() 时创建，避免构造函数就访问硬件

        self.cameras = make_cameras_from_configs(config.cameras)

        self._has_gripper = config.end_effector in ("gripper", "mix")
    ...
    def connect(self, calibrate: bool = True) -> None:
        """连接从臂：创建 S1_arm 实例 → 使能电机 → 连接相机"""
        from S1_SDK import S1_arm, control_mode

        logger.info("正在连接 S1 从臂: dev=%s, version=%s, end_effector=%s",
    ...
    def is_calibrated(self) -> bool:
        # 当前使用 S1 使用自身的零位机制，不需要框架层校准
        return True

    def calibrate(self) -> None:
        """S1 的校准通过 S1_SDK 的 set_zero_position() 完成，框架层不需要额外校准"""
        logger.info("S1 不需要框架层校准。如需重新设置电机零位，请使用 S1_SDK 的 set_zero_position()")
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        if self._has_gripper and "gripper.pos" in action:
            ...
            if self.config.end_effector == "mix":
                self.arm.control_mix(gripper_pos)
            else:
                self.arm.control_gripper(gripper_pos, 0.1)
            ...
            try:
                if self.config.disable_on_disconnect:
                    ...
                    for i, joints in enumerate(trajectory):
                        ...
                        if has_gripper:
                            self.arm.control_gripper(float(gripper_trajectory[i]), 0.1)
                        time.sleep(0.02)
                    self.arm.disable()
                self.arm.close()
            except Exception:
                logger.warning("关闭 S1 从臂时出错，已忽略", exc_info=True)
