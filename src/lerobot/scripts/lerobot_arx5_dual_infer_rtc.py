...
class RTCExecutionState:
    ...
    next_inference_delay_steps: int = DEFAULT_RTC_INFERENCE_DELAY_STEPS
    delay_history: list[int] = dataclasses.field(default_factory=list)


@dataclass
class DebugPauseState:
    enabled: bool = False
    pause_interval: int = 15
    waiting_resume: bool = False
    pause_index: int = 0
    steps_since_resume: int = 0
...
class DebugTraceWriter:
    def __init__(self, root_dir: Path, args: argparse.Namespace) -> None:
        session_name = time.strftime("%Y%m%d_%H%M%S") + f"_{int((time.time() % 1) * 1000):03d}"
        ...
        metadata_path.write_text(json.dumps(_to_jsonable(vars(args)), indent=2), encoding="utf-8")
    ...
    def log_event(self, event_type: str, **payload: Any) -> None:
        ...
        self._file.write(json.dumps(_to_jsonable(record), ensure_ascii=False) + "\n")
        self._file.flush()
...
def _action_chunk_to_actions(
    *,
    ...
    chunk_cpu = action_chunk.to("cpu")
    if chunk_cpu.ndim == 3:
        chunk_cpu = chunk_cpu.squeeze(0)
    elif chunk_cpu.ndim != 2:
        raise ValueError(f"Expected action chunk to have 2 or 3 dims, got shape={tuple(chunk_cpu.shape)}")
...
def _predict_action_chunk_tensor(
    *,
    ...
    return action_chunk_raw, action_chunk
...
def _inference_worker_loop(
    *,
    ...
    while not stop_event.is_set():
        ...
        try:
            raw_action_chunk, action_chunk = _predict_action_chunk_tensor(
            ...
            result = RTCResult(
                ...
                raw_action_chunk=torch.empty((1, 0, len(dataset_features[ACTION]["names"])), dtype=torch.float32),
        ...
        result = RTCResult(
            ...
            raw_action_chunk=raw_action_chunk.detach().to("cpu"),
...
def _log_keyboard_help() -> None:
    logger.info(
        "键盘：[Space] 急停 | [H] 回零 | [R] 执行推理 | [I] debug继续 | [Q] 退出 | [O] open grippers | [P] close grippers"
    )
...
def _reset_execution_state(state: RTCExecutionState) -> None:
    ...
    state.prev_chunk_left_over = None
...
def _reset_debug_pause_state(state: DebugPauseState) -> None:
    state.waiting_resume = False
    state.pause_index = 0
    state.steps_since_resume = 0
...
def _update_auto_inference_delay(
    execution_state: RTCExecutionState,
    *,
    observed_delay_steps: int,
    history_window: int,
) -> int:
    if observed_delay_steps <= 0:
        return execution_state.next_inference_delay_steps
    execution_state.delay_history.append(int(observed_delay_steps))
    if len(execution_state.delay_history) > history_window:
        execution_state.delay_history = execution_state.delay_history[-history_window:]
    execution_state.next_inference_delay_steps = int(np.median(execution_state.delay_history))
    return execution_state.next_inference_delay_steps
...
def _submit_rtc_request(
    *,
    ...
    inference_delay: int,
    dataset_features: dict[str, dict[str, Any]],
    trace_writer: DebugTraceWriter | None = None,
    submit_reason: str = "unspecified",
) -> bool:
    if execution_state.in_flight_request_id is not None:
        return False
    if execution_state.prev_chunk_left_over is not None and (
        execution_state.prev_chunk_left_over.shape[0] != len(execution_state.action_queue)
    ):
        raise RuntimeError(
            "RTC prefix/action queue diverged before request submit: "
            f"{execution_state.prev_chunk_left_over.shape[0]} != {len(execution_state.action_queue)}"
        )
    queue_snapshot = [dict(action) for action in execution_state.action_queue]
    prev_left_over = execution_state.prev_chunk_left_over.clone() if execution_state.prev_chunk_left_over is not None else None
    prev_left_over_actions = (
        _action_chunk_to_actions(
            action_chunk=prev_left_over,
            dataset_features=dataset_features,
            execution_horizon=prev_left_over.shape[0],
        )
    ...
    if trace_writer is not None:
        trace_writer.log_event(
            "rtc_request_submitted",
            submit_reason=submit_reason,
            generation_id=request.generation_id,
            request_id=request.request_id,
            global_step=execution_state.global_step,
            chunk_index=execution_state.chunk_index,
            queue_remaining=len(execution_state.action_queue),
            execution_horizon=execution_horizon,
            inference_delay=inference_delay,
            observation_state=[float(observation[key]) for key in STATE_KEYS],
            prev_chunk_left_over_actions=queue_snapshot,
            prev_chunk_left_over_raw_actions=prev_left_over_actions,
        )
    logger.info(
...
def _consume_ready_rtc_result(
    *,
    ...
    while True:
        ...
        next_inference_delay_steps = execution_state.next_inference_delay_steps
        if auto_delay:
            next_inference_delay_steps = _update_auto_inference_delay(
                execution_state,
                observed_delay_steps=executed_steps_since_submit,
                history_window=auto_delay_window,
            )
        ...
        if result.error is not None:
            if trace_writer is not None:
                trace_writer.log_event(
                    ...
                    global_step=execution_state.global_step,
                    error=result.error,
                    next_inference_delay_steps=next_inference_delay_steps,
                )
            logger.warning(
        ...
        raw_chunk_horizon = result.raw_action_chunk.squeeze(0).detach().to("cpu")[: len(result.actions)]
        raw_left_over = raw_chunk_horizon[real_delay:]
        execution_state.prev_chunk_left_over = raw_left_over if raw_left_over.numel() > 0 else None
        if execution_state.prev_chunk_left_over is not None and (
            execution_state.prev_chunk_left_over.shape[0] != len(execution_state.action_queue)
        ):
            raise RuntimeError(
                "RTC prefix/action queue diverged after merge: "
                f"{execution_state.prev_chunk_left_over.shape[0]} != {len(execution_state.action_queue)}"
            )
        ...
        if trace_writer is not None:
            trace_writer.log_event(
                ...
                request_id=result.request_id,
                infer_latency_s=result.infer_latency_s,
                wall_delay_steps=delay_by_time,
                executed_steps_since_submit=executed_steps_since_submit,
                applied_delay_steps=real_delay,
                global_step=execution_state.global_step,
                chunk_index=execution_state.chunk_index,
                queue_remaining=len(execution_state.action_queue),
                next_inference_delay_steps=next_inference_delay_steps,
                delay_history=execution_state.delay_history,
                current_state=current_state,
                returned_raw_actions=_action_chunk_to_actions(
                    action_chunk=result.raw_action_chunk,
                    dataset_features=dataset_features,
                ...
                merged_raw_actions=(
                    _action_chunk_to_actions(
                        action_chunk=execution_state.prev_chunk_left_over,
                        dataset_features=dataset_features,
                        execution_horizon=int(execution_state.prev_chunk_left_over.shape[0]),
                    )
                    if execution_state.prev_chunk_left_over is not None
                    else []
                ),
                merged_actions=execution_state.action_queue,
            )
        logger.info(
            "RTC result merged: request=%d latency=%.4fs wall_delay=%d step_delay=%d applied_delay=%d new_queue=%d next_delay=%d",
            result.request_id,
...
def main() -> None:
    ...
    parser.add_argument("--rtc-auto-delay", action="store_true")
    parser.add_argument("--rtc-auto-delay-window", type=int, default=3)
    parser.add_argument(
    ...
    parser.add_argument("--debug-pause-interval", type=int, default=15)
    parser.add_argument("--debug-log-dir", type=Path, default=Path("logs/rtc_debug"))
    ...
    if args.rtc_auto_delay_window <= 0:
        parser.error("--rtc-auto-delay-window 必须为正数。")
    if args.rtc_max_guidance_weight <= 0:
        parser.error("--rtc-max-guidance-weight 必须为正数。")
    if args.debug_pause_interval <= 0:
        parser.error("--debug-pause-interval 必须为正数。")
    if args.debug_mode and args.no_keyboard:
        parser.error("--debug-mode 依赖键盘控制，不能与 --no-keyboard 一起使用。")
    ...
    execution_state = RTCExecutionState()
    execution_state.next_inference_delay_steps = int(args.rtc_inference_delay_steps)
    debug_pause_state = DebugPauseState(enabled=args.debug_mode, pause_interval=args.debug_pause_interval)
    ...
    trace_writer = DebugTraceWriter(args.debug_log_dir, args) if args.debug_mode else None
    ...
    try:
        ...
        if keyboard is not None:
            ...
            if args.debug_mode:
                logger.info(
                    "DEBUG MODE 已启用：每次恢复后执行 %d 个动作就暂停，RTC 在执行期间继续异步推理。按 [I] 继续。日志保存到 %s",
                    args.debug_pause_interval,
                    trace_writer.session_dir if trace_writer is not None else args.debug_log_dir,
                )
        else:
        ...
        while running:
            if keyboard is not None:
                ...
                if debug_pause_state.waiting_resume and key == "i":
                    debug_pause_state.waiting_resume = False
                    debug_pause_state.steps_since_resume = 0
                    logger.info("DEBUG MODE：继续执行接下来的 %d 个动作。", args.debug_pause_interval)
                    if trace_writer is not None:
                        trace_writer.log_event(
                            "debug_resume",
                            global_step=execution_state.global_step,
                            chunk_index=execution_state.chunk_index,
                            queue_remaining=len(execution_state.action_queue),
                            pause_index=debug_pause_state.pause_index,
                            in_flight_request_id=execution_state.in_flight_request_id,
                        )
                    key = None
                ...
                if previous_state != LoopState.RUNNING and state == LoopState.RUNNING:
                    ...
                    _reset_debug_pause_state(debug_pause_state)
                    if trace_writer is not None:
                        trace_writer.log_event("run_started", global_step=execution_state.global_step)
                    ...
                    right_arm.hold_position()
                    if trace_writer is not None:
                        trace_writer.log_event("run_stopped", global_step=execution_state.global_step)
            if not running:
                break
            ...
            _consume_ready_rtc_result(
                ...
                auto_delay=args.rtc_auto_delay,
                auto_delay_window=args.rtc_auto_delay_window,
                trace_writer=trace_writer,
            )

            if debug_pause_state.waiting_resume:
                time.sleep(0.05)
                continue
            ...
            if not execution_state.action_queue and execution_state.in_flight_request_id is None:
                ...
                _submit_rtc_request(
                    ...
                    inference_delay=execution_state.next_inference_delay_steps,
                    dataset_features=dataset_features,
                    trace_writer=trace_writer,
                    submit_reason="queue_empty_bootstrap",
                )
            ...
            if execution_state.action_queue:
                ...
                if execution_state.prev_chunk_left_over is not None and (
                    ...
                    raise RuntimeError(
                        "RTC prefix/action queue diverged before action execution: "
                        f"{execution_state.prev_chunk_left_over.shape[0]} != {len(execution_state.action_queue)}"
                    )
                action = execution_state.action_queue.pop(0)
                raw_action = None
                if execution_state.prev_chunk_left_over is not None:
                    raw_action = _action_chunk_to_actions(
                        action_chunk=execution_state.prev_chunk_left_over[:1],
                        dataset_features=dataset_features,
                        execution_horizon=1,
                    )[0]
                    remaining_prefix = execution_state.prev_chunk_left_over[1:]
                    execution_state.prev_chunk_left_over = remaining_prefix if remaining_prefix.numel() > 0 else None
                ...
                if debug_pause_state.enabled:
                    debug_pause_state.steps_since_resume += 1
                ...
                if (
                    debug_pause_state.enabled
                    and debug_pause_state.steps_since_resume >= debug_pause_state.pause_interval
                ):
                    debug_pause_state.waiting_resume = True
                    debug_pause_state.pause_index += 1
                    left_arm.hold_position()
                    right_arm.hold_position()
                    logger.info(
                        "DEBUG MODE：已执行 %d 个动作，暂停等待 [I]。当前队列剩余=%d，in_flight=%s",
                        debug_pause_state.steps_since_resume,
                        len(execution_state.action_queue),
                        execution_state.in_flight_request_id,
                    )
                    if trace_writer is not None:
                        trace_writer.log_event(
                            ...
                            in_flight_request_id=execution_state.in_flight_request_id,
                            pause_index=debug_pause_state.pause_index,
                            steps_since_resume=debug_pause_state.steps_since_resume,
                        )
            else:
            ...
            if (
                ...
                _submit_rtc_request(
                    ...
                    dataset_features=dataset_features,
                    trace_writer=trace_writer,
                    submit_reason="queue_low_prefetch",
                )
    finally:
        ...
        if trace_writer is not None:
            trace_writer.log_event(
                ...
                global_step=execution_state.global_step,
                chunk_index=execution_state.chunk_index,
                camera_failure=str(camera_failure) if camera_failure is not None else None,
            )
            trace_writer.close()
