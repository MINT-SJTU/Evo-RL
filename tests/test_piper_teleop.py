...
from lerobot.scripts.lerobot_teleoperate import (
    _make_fake_inference_action,
    _make_fake_inference_leader_follower_config,
    teleop_loop,
)
...
def test_piper_leader_teaching_mode_allows_missing_status(monkeypatch):
    patch_fake_sdk(monkeypatch)
    seen = {}

    def fake_read_piper_ctrl_mode(arm, timeout_s=1.0, poll_s=0.02):
        del arm, poll_s
        seen["timeout_s"] = timeout_s
        return None

    monkeypatch.setattr(piper_leader_module, "read_piper_ctrl_mode", fake_read_piper_ctrl_mode)

    teleop = PiperLeader(
        PiperLeaderConfig(
            port="can1",
            read_only=True,
            allow_teaching_mode=True,
            teaching_mode_read_timeout_s=2.0,
...
def test_piper_leader_read_only_prefers_leader_ctrl_frames(monkeypatch):
    patch_fake_sdk(monkeypatch)
    monkeypatch.setattr(piper_leader_module, "read_piper_ctrl_mode", lambda *args, **kwargs: None)
...
def test_piper_leader_read_only_requires_leader_ctrl_frames(monkeypatch):
    patch_fake_sdk(monkeypatch)
    monkeypatch.setattr(piper_leader_module, "read_piper_ctrl_mode", lambda *args, **kwargs: None)
    ...
    teleop = PiperLeader(
        PiperLeaderConfig(
            ...
            prefer_ctrl_messages=True,
            fallback_to_feedback=False,
            read_only_action_timeout_s=0.01,
            require_calibration=False,
        )
    )
    ...
    teleop.connect(calibrate=False)
    try:
        with pytest.raises(RuntimeError, match="no leader joint control frames"):
            teleop.get_action()
    finally:
...
def test_bimanual_piper_read_only_returns_side_that_has_first_frame(monkeypatch):
    patch_fake_sdk(monkeypatch)
    ...
    teleop.connect(calibrate=False)
    try:
        teleop.left_arm.arm._joint_ctrl.time_stamp = 0.0
        teleop.right_arm.arm._joint_ctrl.time_stamp = 0.0
        action = teleop.get_action()
        assert action == {}

        teleop.left_arm.arm._joint_ctrl.time_stamp = 1.0
        action = teleop.get_action()
        assert "left_joint_1.pos" in action
        assert "right_joint_1.pos" not in action

        teleop.right_arm.arm._joint_ctrl.time_stamp = 1.0
        action = teleop.get_action()
        assert "left_joint_1.pos" in action
        assert "right_joint_1.pos" in action
    finally:
...
def test_fake_inference_action_sweeps_base_joint_and_holds_other_joints():
    hold_action = {
        "left_joint_1.pos": 10.0,
        "left_joint_2.pos": 20.0,
        "left_gripper.pos": 30.0,
        "right_joint_1.pos": -30.0,
        "right_joint_6.pos": 40.0,
        "joint_1.pos": 5.0,
        "joint_3.pos": 50.0,
    }

    action = _make_fake_inference_action(
        hold_action,
        elapsed_s=2.5,
        total_swing_deg=120.0,
        period_s=10.0,
    )

    assert action["left_joint_1.pos"] == pytest.approx(70.0)
    assert action["right_joint_1.pos"] == pytest.approx(30.0)
    assert action["joint_1.pos"] == pytest.approx(65.0)
    assert action["left_joint_2.pos"] == 20.0
    assert action["left_gripper.pos"] == 30.0
    assert action["right_joint_6.pos"] == 40.0
    assert action["joint_3.pos"] == 50.0
    assert hold_action["left_joint_1.pos"] == 10.0


def test_fake_inference_builds_leader_follower_config_from_bimanual_piperx_leader():
    cfg = _make_fake_inference_leader_follower_config(
        BiPiperXLeaderConfig(
            id="leaders",
            left_arm_config=PiperXLeaderConfigBase(
                port="can2",
                command_high_follow=False,
                sync_gripper=True,
            ),
            right_arm_config=PiperXLeaderConfigBase(
                port="can3",
                command_high_follow=True,
                sync_gripper=False,
            ),
            process_isolation=False,
        ),
    ...
    assert isinstance(cfg, BiPiperXFollowerConfig)
    assert cfg.id == "leaders_fake_inference_followers"
    assert cfg.left_arm_config.port == "can2"
    assert cfg.right_arm_config.port == "can3"
    assert cfg.left_arm_config.require_calibration is False
    assert cfg.right_arm_config.require_calibration is False
    assert cfg.left_arm_config.speed_ratio == 15
    assert cfg.right_arm_config.speed_ratio == 15
    assert cfg.left_arm_config.high_follow is False
    assert cfg.right_arm_config.high_follow is True
    assert cfg.left_arm_config.sync_gripper is True
    assert cfg.right_arm_config.sync_gripper is False


def test_teleop_loop_fake_inference_controls_leader_followers(monkeypatch):
    class FakeKeyReader:
    ...
    class FakeRobot:
        action_features = {
            **{f"left_joint_{idx}.pos": float for idx in range(1, 7)},
            **{f"right_joint_{idx}.pos": float for idx in range(1, 7)},
        }
    ...
    class FakeLeaderFollowerController:
        def __init__(self):
    ...
    monkeypatch.setattr(teleoperate_module, "_NonBlockingKeyReader", FakeKeyReader)

    _, robot_action_processor, robot_observation_processor = make_default_processors()
    teleop_action_processor, _, _ = make_default_processors()
    robot = FakeRobot()
    leader_followers = FakeLeaderFollowerController()
    ...
    teleop_loop(
        ...
        fake_inference_total_swing_deg=120.0,
        fake_inference_period_s=10.0,
        fake_inference_leader_follower_controller=leader_followers,
    )

    assert leader_followers.connect_calls == 1
    assert leader_followers.disconnect_calls == 1
    assert leader_followers.sent_actions
    assert len(robot.sent_actions) >= 2
    assert leader_followers.sent_actions[-1]["left_joint_2.pos"] == 2.0
    assert leader_followers.sent_actions[-1]["right_joint_6.pos"] == -6.0


def test_teleop_loop_fake_inference_waits_for_missing_side(monkeypatch):
    class FakeKeyReader:
        def __init__(self, enabled: bool):
            assert enabled is True
            self.keys = ["i"]
    ...
    class FakeTeleop:
        def __init__(self):
            self.calls = 0
            self.left_action = {f"left_joint_{idx}.pos": float(idx) for idx in range(1, 7)}
            self.full_action = {
                **self.left_action,
                **{f"right_joint_{idx}.pos": float(-idx) for idx in range(1, 7)},
            }
        ...
        def get_action(self):
            self.calls += 1
            if self.calls == 1:
                return dict(self.left_action)
            return dict(self.full_action)
