...
if [[ "$NUM_MACHINES" -lt 1 ]]; then
  echo "[ERROR] NODE_HOSTS_CSV 至少需要 1 台机器，当前是 $NUM_MACHINES: $NODE_HOSTS_CSV"
  exit 1
fi
...
for idx in "${!NODE_HOSTS[@]}"; do
  ...
  ssh_host "$host" "mkdir -p '$LOG_DIR' && cd '$PROJECT_DIR' && ( nohup bash -lc \"cd '$PROJECT_DIR' && exec env RUN_ID='$RUN_ID' RESUME='$RESUME' NODE_RANK='$idx' MAIN_PROCESS_IP='$MAIN_PROCESS_IP' MAIN_PROCESS_PORT='$MAIN_PROCESS_PORT' NUM_MACHINES='$NUM_MACHINES' NUM_PROCESSES='$NUM_PROCESSES' CONDA_ENV_NAME='$CONDA_ENV_NAME' WANDB_MODE='$WANDB_MODE' WANDB_ENABLE='$WANDB_ENABLE' WANDB_DISABLE_ARTIFACT='$WANDB_DISABLE_ARTIFACT' TRAIN_STEPS='$TRAIN_STEPS' BATCH_SIZE='$BATCH_SIZE' LOG_FREQ='$LOG_FREQ' SAVE_FREQ='$SAVE_FREQ' OPTIMIZER_LR='$OPTIMIZER_LR' SCHEDULER_DECAY_LR='$SCHEDULER_DECAY_LR' DATASET_ROOT='$DATASET_ROOT' DATASET_REPO_ID='$DATASET_REPO_ID' POLICY_REPO_ID='$POLICY_REPO_ID' POLICY_COMPILE='$POLICY_COMPILE' POLICY_COMPILE_MODE='$POLICY_COMPILE_MODE' GRADIENT_CHECKPOINTING='$GRADIENT_CHECKPOINTING' POLICY_PUSH_TO_HUB='$POLICY_PUSH_TO_HUB' WEIGHTED_BC_ENABLE='$WEIGHTED_BC_ENABLE' HF_CACHE_DIR='$HF_CACHE_DIR' HF_LOCAL_FILES_ONLY='$HF_LOCAL_FILES_ONLY'$OPTIONAL_DISTRIBUTED_ENV_STR bash '$LAUNCH_NAME'\" > '$log' 2>&1 < /dev/null & ) ; disown -a >/dev/null 2>&1 || true"
done
